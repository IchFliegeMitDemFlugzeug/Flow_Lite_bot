...
from ...keyboards.inline_mode.transfer_actions import (        # Импортируем сборщик инлайн-клавиатуры
    build_transfer_actions_keyboard,                           # Функция, которая вернёт клавиатуру с кнопкой "Перевести"
)
...
def _build_inline_article(
    option: InlinePaymentOption,                                             # Один вариант (телефон или карта)
    parsed: ParsedInlineQuery,                                              # Результат парсинга запроса
    result_id: str,                                                         # Уникальный ID для этого результата
) -> InlineQueryResultArticle:
    """
    Превращаем наш внутренний вариант (телефон/карта) в объект InlineQueryResultArticle.

    Здесь:
    - title / description — то, что видно в ЗЕЛЁНОМ списке;
    - input_message_content — текст, который в итоге попадёт в чат;
    - reply_markup — инлайн-клавиатура с кнопкой "Перевести".
    """

    # Строим текст сообщения, который окажется в чате после выбора варианта
    message_text: str = build_transfer_message_text(                       # Вызываем текстовый конструктор
        option=option,                                                     # Передаём данные по выбранному реквизиту
        parsed_query=parsed,                                               # И результат разбора inline-запроса (сумма, банки и т.п.)
    )

    # Оборачиваем текст в InputTextMessageContent — это "тело" сообщения,
    # которое Telegram отправит в ЧАТ после выбора варианта.
    input_content: InputTextMessageContent = InputTextMessageContent(      # Создаём объект содержимого сообщения
        message_text=message_text,                                         # Обязательный параметр — сам текст сообщения
        parse_mode="Markdown",                                             # Включаем Markdown, чтобы работали `моно` и т.п.
    )

    # Строим инлайн-клавиатуру с кнопкой "Перевести"
    reply_markup = build_transfer_actions_keyboard(                        # Получаем объект InlineKeyboardMarkup
        option=option,                                                     # Передаём вариант реквизита (на будущее, для диплинков)
        parsed_query=parsed,                                               # И разобранный запрос (можно будет использовать в ссылке)
    )
    ...
    article: InlineQueryResultArticle = InlineQueryResultArticle(
        ...
        reply_markup=reply_markup,                                         # Инлайн-клавиатура с кнопкой "Перевести"
        # thumb_url / thumb_width / thumb_height можно будет добавить позже для превью-картинки.
    )
