...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    3) Сбрасываем FSM.
    4) Если регистрация завершена — отправляем "Личный кабинет".
    5) Если незавершена, но есть сохранённый шаг — восстанавливаем его.
    6) Иначе показываем стартовый экран с документами.
    """

    update_basic_user_info(                                # Обновляем базовые данные в JSON-файле
    ...
    await state.clear()                                    # Полностью очищаем FSM (состояние и данные)

    user_id = message.from_user.id                         # Сохраняем user_id в локальную переменную

    if _is_user_registered(user_id):                       # Если регистрация полностью завершена
        sent_message = await send_message_with_headline(   # Отправляем заглушку "Личный кабинет"
            message=message,
            text="Личный кабинет",
            headline_type=HEADLINE_BASE,
            reply_markup=None,
    ...
    # Пытаемся восстановить шаг регистрации из файловой БД
    restored = await _restore_registration_step_from_db(
        message=message,
        state=state,
    )
    if restored:                                           # Если шаг удалось восстановить
        return                                             # Больше ничего делать не нужно

    # Если ничего не восстановили — показываем стартовый экран
...
@registration_router.callback_query(                        # Хэндлер для callback-запросов
    RegistrationStates.waiting_for_main_bank,               # Срабатывает только в состоянии waiting_for_main_bank
...
async def process_main_bank_choice(
    ...
    data: str = callback.data                              # Например, "main_bank:sber"
    _, action = data.split(":", maxsplit=1)                # Отделяем префикс "main_bank" от значения

    fsm_data: dict = await state.get_data()                # Текущее содержимое FSM

    user_id = callback.from_user.id                        # Берём user_id

    phone = _get_phone_from_state_or_db(user_id, fsm_data) # Снова аккуратно достаём номер из FSM/БД
    if not phone:
    ...
    main_bank: str | None = fsm_data.get("main_bank")      # Текущий основной банк (может быть None)

    # --- Кнопка "Назад" --- #
    if action == "back":
        set_registration_progress(user_id, "banks", phone)  # В БД: откат на шаг "banks"

        await state.set_state(RegistrationStates.waiting_for_banks)  # FSM в состояние выбора банков

        text = BANK_CHOICE_TEXT_TEMPLATE.format(phone=phone)         # Текст шага 2

        keyboard = build_bank_choice_keyboard(                       # Клавиатура выбора банков
            selected_banks=selected_banks,
        )

        await callback.answer()                                      # Закрываем "часики"

        await edit_message_with_headline(                            # Редактируем сообщение обратно на шаг выбора банков
            message=callback.message,
            text=text,
            headline_type=HEADLINE_REG_2,
            reply_markup=keyboard,
    ...
    if action == "next":
        add_or_update_phone(                                       # Сохраняем банки и основной банк в БД
            user_id=user_id,
            phone=phone,
            banks=selected_banks,
            main_bank=main_bank,
        ...
        set_registration_progress(user_id, "completed", None)      # В БД: регистрация завершена

        final_text: str = BANK_CHOICE_DONE_TEXT_TEMPLATE           # Финальный текст
        ...
        sent_message = await send_message_with_headline(           # Отправляем финальное сообщение
            message=callback.message,
            text=final_text,
            headline_type=HEADLINE_BASE,
        ...
        await state.update_data(last_bot_message_id=sent_message.message_id)
        await state.clear()                                        # Чистим FSM
        return

    # --- Выбор конкретного основного банка --- #
    bank_code: str = action                                        # В этом случае action — код банка

    main_bank = bank_code                                          # Считаем этот банк новым основным
    ...
    add_or_update_phone(                                           # Синхронизируем БД
        user_id=user_id,
        phone=phone,
        banks=selected_banks,
        main_bank=main_bank,
    ...
    new_keyboard = build_main_bank_choice_keyboard(                # Перестраиваем клавиатуру
        available_banks=selected_banks,
        main_bank=main_bank,
    )
    ...
    await safe_edit_reply_markup(                                  # Обновляем только клавиатуру
        callback.message,
        reply_markup=new_keyboard,
    )
