...
import os  # os — чтобы читать переменные окружения (можно отключить запись в БД на время тестов)
import logging  # logging — чтобы писать предупреждения в лог вместо падения
from sqlalchemy.exc import SQLAlchemyError  # SQLAlchemyError — базовый класс ошибок SQLAlchemy (ловим всё, что может прилететь от БД)
...
# ---------------------------------------------------------------------------
# INLINE MODE: хранение ключей startapp и факта открытия WebApp
# ---------------------------------------------------------------------------

def inline_mode_upsert_webapp_session(
    *,  # * — заставляем вызывать функцию только именованными аргументами (так меньше шансов перепутать поля)
    transfer_id: str,  # transfer_id — ключ операции (то, что кладём в startapp=...)
    creator_tg_user_id: Optional[int] = None,  # creator_tg_user_id — TG id того, кто сформировал инлайн-результат
    context_json: Optional[str] = None,  # context_json — JSON с контекстом (raw_query + parsed + option)
    opener_tg_user_id: Optional[int] = None,  # opener_tg_user_id — TG id того, кто открыл WebApp (позже заполнится backend'ом)
    opener_json: Optional[str] = None,  # opener_json — JSON с полной инфой о юзере-открывателе (пишет backend)
    raw_init_data: Optional[str] = None,  # raw_init_data — сырая initData строка Telegram (подписанная)
) -> None:
    """
    Безопасная запись данных inline-mode в БД.

    Что изменено по твоей просьбе:
    - ЛОГИРУЕМ то, что функция пытается записать в БД (payload + ключ),
      чтобы можно было тестировать и видеть факт попытки записи даже без реально созданной таблицы.
    - Ошибки БД НЕ роняют бота.
    - Если хочешь полностью отключить запись для теста: INLINE_MODE_DB_DISABLE=1
    """

    # ----------------------------
    # 0) Возможность быстро отключить запись в БД (для тестов)
    # ----------------------------
    if os.getenv("INLINE_MODE_DB_DISABLE", "0") == "1":  # Читаем переменную окружения; если "1" — запись отключена
        logger = logging.getLogger(__name__)  # Создаём/получаем логгер (на этом шаге нам нужен лог)
        logger.warning(  # warning — чтобы точно было видно даже если общий уровень логов WARNING
            "INLINE MODE DB: запись отключена env INLINE_MODE_DB_DISABLE=1. transfer_id=%s",  # Сообщение
            transfer_id,  # Подставляем ключ операции
        )
        return  # Выходим, не обращаясь к БД

    # ----------------------------
    # 1) Подготовка логгера
    # ----------------------------
    logger = logging.getLogger(__name__)  # Получаем логгер для этого модуля

    # ----------------------------
    # 2) Подготовим “короткие версии” больших полей для лога
    # ----------------------------
    def _short(text_value: Optional[str], limit: int = 400) -> Optional[str]:
        """Обрезаем длинные строки для лога, чтобы консоль не превращалась в простыню."""
        if text_value is None:  # Если строка отсутствует
            return None  # Возвращаем None
        if len(text_value) <= limit:  # Если строка короткая
            return text_value  # Возвращаем как есть
        return text_value[:limit] + f"... (truncated, len={len(text_value)})"  # Возвращаем обрезанную версию

    # ----------------------------
    # 3) Логируем то, что собираемся передать в БД (ВАЖНО: до реального SQL)
    # ----------------------------
    logger.warning(  # warning — чтобы было видно без настройки INFO
        "INLINE MODE DB: попытка UPSERT. transfer_id=%s creator_tg_user_id=%s opener_tg_user_id=%s "
        "context_json=%r opener_json=%r raw_init_data=%r",
        transfer_id,  # Ключ операции
        creator_tg_user_id,  # Кто создал
        opener_tg_user_id,  # Кто открыл (если уже есть)
        _short(context_json),  # Контекст операции (обрезанный)
        _short(opener_json),  # Данные opener (обрезанные)
        _short(raw_init_data),  # initData (обрезанная)
    )

    # ----------------------------
    # 4) Готовим SQL UPSERT (MySQL)
    # ----------------------------
    sql = text(
        """
        INSERT INTO inline_webapp_sessions
            (transfer_id, creator_tg_user_id, context_json, opener_tg_user_id, opener_json, raw_init_data, opened_at)
        VALUES
            (:transfer_id, :creator_tg_user_id, :context_json, :opener_tg_user_id, :opener_json, :raw_init_data,
             CASE WHEN :opener_tg_user_id IS NULL THEN NULL ELSE CURRENT_TIMESTAMP END)
        ON DUPLICATE KEY UPDATE
            creator_tg_user_id = COALESCE(VALUES(creator_tg_user_id), creator_tg_user_id),
            ...
            opened_at          = COALESCE(VALUES(opened_at),          opened_at);
        """
    )

    # ----------------------------
    # 5) Параметры запроса (безопасная подстановка)
    # ----------------------------
    params = {
        "transfer_id": transfer_id,
        "creator_tg_user_id": creator_tg_user_id,
    ...
    # ----------------------------
    # 6) Выполняем запрос “мягко”: любые ошибки БД ловим и не роняем бота
    # ----------------------------
    try:
        with get_session() as session:  # Открываем сессию SQLAlchemy
            session.execute(sql, params)  # Выполняем запрос (UPSERT)
        logger.warning(  # warning — чтобы точно было видно в консоли
            "INLINE MODE DB: UPSERT выполнен (или отправлен в транзакцию) успешно. transfer_id=%s",
            transfer_id,
        )
    except SQLAlchemyError as exc:
        logger.warning(
            "INLINE MODE DB: ошибка при UPSERT (не фатально, бот продолжает работу). transfer_id=%s error=%s",
            transfer_id,
            exc,
        )
        return
        ...
        logger.warning(
            "INLINE MODE DB: неожиданная ошибка при UPSERT (не фатально). transfer_id=%s error=%s",
            transfer_id,
            exc,
        )
        return
